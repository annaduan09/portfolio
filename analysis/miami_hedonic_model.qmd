...
### Outlier Analysis
```{r outlier analysis}
houses_known$top_5_pct <- ifelse(houses_known$price > quantile(houses_known$price, 0.95), 1, 0)

houses_known %>%
  st_drop_geometry() %>%
  group_by(top_5_pct) %>%
  summarise(price = median(price), 
            medhhinc = median(medHHInc),
            actual_sqft = median(actual_sqft),
            lot_size = median(lot_size),
            distWater = median(distWater),
            n = n()) %>%
  pander()
```
