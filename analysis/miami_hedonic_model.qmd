...
ggplot() +
  ...
  labs(title = "Distribution of House Prices in Miami-Dade County", caption = "filtered for price < $3 million (95th percentile)", color = "Sale Price ($1k)") +
...
for(i in vars){
  plotList[[i]] <- 
ggplot(houses_cat, aes(x = .data[[i]], y = price)) +
...
miami_tract <- tracts(state = "FL", county = "Miami-Dade") %>%
  st_transform('ESRI:102658') %>% # NAD 1983 StatePlane Florida East FIPS 0901 Feet
  filter(substr(GEOID, 1, 6) == "120860") %>%
  erase_water() %>%
  dplyr::select(GEOID)
...
First, we use a correlation matrix to get an idea of different external variables' correlations with sale price. Below is a list of positive and negative correlations:
...
ggcorrplot(
  ...
  lab_col = "white",
  insig = "blank") +  
    labs(title = "Sale Price Correlation with Numeric Variables", x = "", y = "") +
  theme_minimal() +
...
ggplot(data = housesKnown, aes(x = pctVacant, y = price)) +
  geom_point(alpha = 0.5, color = "lightblue")  +
...
ggplot() +
  ...
  geom_sf(data = housesKnown, aes(colour = q5(price))) +
