...
houses <- st_read("/Users/annaduan/Desktop/GitHub/Miami-Home-Sales-Prediction/Raw\ Data/studentsData.geojson") %>%
  ...
  st_transform('ESRI:102658') %>%
  st_centroid() %>%
  mutate(sale_type = as.factor(saleType),
         city = as.factor(Property.City),
         zip_code = as.factor(Property.Zip),
         landuse = as.factor(Land.Use),
  ...
  dplyr::select(Folio, address, sale_type, city, zip_code, landuse, zoning, sqft, lot_size, beds, baths, stories, units, year_built, eff_year_built, feature_1, feature_2, feature_3, living_sqft, actual_sqft, SalePrice)
...
correlation.long <- houses %>%
  st_drop_geometry() %>%
    dplyr::select(-Folio, -address, -sale_type, -city, -zip_code, -landuse, -zoning, -feature_1, -feature_2, -feature_3) %>%
...
correlation.cor <-
  correlation.long %>%
    group_by(Variable) %>%
    summarize(correlation = cor(Value, SalePrice, use = "complete.obs"))


ggplot(correlation.long, aes(Value, SalePrice)) +
  geom_point(size = 0.1) +
...
for(i in vars){
  histList[[i]] <- 
    ggplot() +
      geom_histogram(data = houses, aes(x = .data[[i]]), stat = "count") +
      labs(title=i) +
      theme_minimal() +
      theme(plot.title = element_text(size=10))
}

do.call(grid.arrange,c(histList, ncol = 3, top = "Feature Distributions"))
...
# Custom function to check for features
check_feature <- function(df, feature_names, columns) {
  feature_present <- purrr::map_lgl(feature_names, ~rowSums(stringr::str_detect(df[columns], .x)) > 0)
  ifelse(feature_present, paste(feature_names[1], sep=""), paste("No", feature_names[1], sep=" "))
}

# Apply the function for each feature
houses <- houses %>%
  mutate(
    Pool = check_feature(houses, c("Pool", "Whirlpool", "Jacuzzi"), c("XF1", "XF2", "XF3")),
    Patio = check_feature(houses, "Patio", c("XF1", "XF2", "XF3")),
    Fence = check_feature(houses, "Fence", c("XF1", "XF2", "XF3")),
    Gazebo = check_feature(houses, "Gazebo", c("XF1", "XF2", "XF3")),
    Carport = check_feature(houses, "Carport", c("XF1", "XF2", "XF3")),
    Wall = check_feature(houses, "Wall", c("XF1", "XF2", "XF3")),
    Dock = check_feature(houses, "Dock", c("XF1", "XF2", "XF3"))
  )
...
```{r Figure 1: Correlation Matrix, fig.height=10, fig.width=10, message=FALSE, warning=FALSE}
...
```{r Fig 2.1-2.4: 4 SCATTERPLOT home price correlation, message=FALSE, warning=FALSE}
housesKnown <- houses %>%    
  filter(.,toPredict == 0)
...
```{r Fig 4.1-4.5: MAP 3 Independent Variables, fig.height=5, fig.width=8, message=FALSE, warning=FALSE, results='hide'}
