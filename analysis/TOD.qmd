...
################################PULL TRANSIT DATA################################
# Read file
mbtaNode <- st_read("mbta_node.geojson") %>%
...
ggplot(allTracts.group %>% filter(!is.na(medHHInc)))+
  geom_sf(data = st_union(allTractsBos), color = "transparent")+
  geom_sf(aes(fill = q5(medHHInc)), color = "transparent") +
  geom_sf(data = buffer, fill = "transparent", color = "red")+
  scale_fill_brewer(palette = "Blues",
                    labels = qBr(allTracts.group, "medHHInc"),
                    name = "Median HH Income\n(Quintile Breaks, TOD in Red)") +
  labs(title = "Median Household Income", subtitle = "Boston, MA (US Dollars)", caption="Figure 3.3") +
  facet_wrap(~year)+
  theme_void() + 
...
allTracts.Summary %>%
  ...
  pander(caption = "Table 3.1: Neighborhood Characteristics by TOD and Year")
```
...
allTracts.Summary %>%
  ...
  ggplot(aes(year, Value, fill = TOD)) +
    ...
    scale_fill_manual(values = c("dodgerblue", "dodgerblue4"), name = "") +
...
TOD tracts seem to have more robberies, particularly downtown. Even when
...
# Read crime data
crime12 <- read.csv("Crime12.csv", header = TRUE, sep = ",", quote = "\"",
         dec = ".")
crime18 <- read.csv("Crime18.csv", header = TRUE, sep = ",", quote = "\"",
                       dec = ".")
...
ggplot() +   
  geom_sf(data = bosTracts, fill = "gray90", color = "transparent") +
...
allTracts.fiveMarkets.Summary %>%
  ...
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~Variable, scales = "free", ncol=5) +
  scale_fill_manual(values = c("chartreuse", "cyan", "black", "gray", "orange", "pink", "#CDCDCD")) +
...
MBTA: https://docs.digital.mass.gov/dataset/massgis-data-trains
